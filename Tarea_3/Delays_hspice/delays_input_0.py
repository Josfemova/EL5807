...
data = pd.read_csv('delays_1_to_0/delays_1_to_0.csv', delim_whitespace=True)
data_hold = pd.read_csv('delays_1_to_0_hold/delays_1_to_0_hold.csv', delim_whitespace=True)
...
slope = -1
x_tangent = np.linspace(tDC_slope_neg1 - 5e-11, tDC_slope_neg1 + 5e-11, 100)
...
plt.plot(tDC, tCQ, marker='o', label='tCQ: D falls, Q falls')
plt.plot(tDC_hold, tCQ_hold, marker='o', label='tCQ: D rises, Q falls')
...
plt.axvline(x=-5e-11, color='black', linestyle='--', linewidth=1, label='-t_hold = 5.0e-11')

plt.axhline(y=tCQ_slope_neg1, color='b', linestyle='--', linewidth=1, label=f't_pcq = {tCQ_slope_neg1}')

plt.title('Flip-flop setup and hold times for Input 0')
plt.xlabel('tDC')
plt.ylabel('Delays')
plt.grid(True)
plt.legend()
